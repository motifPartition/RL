	...
	for i in range(episodes):
		...
		for idx, graph in enumerate(training_dataset):
			...
			while time < len_episode:
				...
				_, va, vb, ca, cb = volumes_2(start_all)

				nnz = start_all.num_edges
				start.x[:, 3] = torch.true_divide(va, nnz)
				start.x[:, 4] = torch.true_divide(vb, nnz)

				start.x[:, 5] = torch.true_divide(ca, counts)
				start.x[:, 6] = torch.true_divide(cb, counts)
...
if __name__ == "__main__":
	...
	if dataset_type == 'delaunay':
		torch.save(model.state_dict(), outdir + 'model_partitioning_delaunay_basic')
	else:
		torch.save(
		    model.state_dict(),
		    outdir +
