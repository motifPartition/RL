...
def k_hop_graph_cut(graph, k):
	...
	subprocess.call(["../Snap-6.0/examples/motifs/motifs", "-i:./graph_.txt", f"-m:{3}", f"-o:all"])
...
def volumes_2(graph):
	...
	file_1 = open('graph_a_.txt','w')
	file_2 = open('graph_b_.txt','w')
	...
	subprocess.call(["../Snap-6.0/examples/motifs/motifs", "-i:./graph_a_.txt", f"-m:{3}", f"-o:ia_"])
	ia_df = pd.read_csv('ia_-counts.tab',sep='\t')
	counts_ia = ia_df['Count'].sum()

	subprocess.call(["../Snap-6.0/examples/motifs/motifs", "-i:./graph_b_.txt", f"-m:{3}", f"-o:ib_"])
	ib_df = pd.read_csv('ib_-counts.tab',sep='\t')
	...
	for i in range(episodes):
		...
		for idx, graph in enumerate(training_dataset):
			...
			while time < len_episode:
				...
				rew = reward_NC_2(start_all, positions[action])
				rew_partial += rew
			...
			if p % print_loss == 0:
				...
				torch.save(all_rew, 'rews_new_partition')
				torch.save(all_vals, 'vals_partition')
				torch.save(all_log, 'logs_partition')
...
if __name__ == "__main__":
	...
	if dataset_type == 'delaunay':
		torch.save(model.state_dict(), outdir + 'model_partitioning_delaunay_new_partition')
	else:
		torch.save(
		    model.state_dict(),
		    outdir +
